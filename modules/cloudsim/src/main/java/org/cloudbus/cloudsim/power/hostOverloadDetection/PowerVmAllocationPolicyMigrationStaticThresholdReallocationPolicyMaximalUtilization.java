 ...
 */
public class PowerVmAllocationPolicyMigrationStaticThresholdReallocationPolicyMaximalUtilization extends PowerVmAllocationPolicyMigrationStaticThreshold {
	 ...
	 * @param activeFirst
	 */
	public PowerVmAllocationPolicyMigrationStaticThresholdReallocationPolicyMaximalUtilization(
	...
	public PowerHost findHostForVmFromPassed(Vm vm, List<? extends Host> possibleDestinationHosts, Set<? extends Host> excludedHosts) {
		...
		for (Host h : possibleDestinationHosts) {
			...
			if (host.isSuitableForVm(vm)) {
				...
				try {
					double destinationUtilization = host.getUtilizationOfCpu();

					if (destinationUtilization > maxUtilization) {
						maxUtilization = destinationUtilization;
						allocatedHost = host;
					}

				} catch (Exception e) {
					Log.printConcatLine("asdfgh");
				}
			}
		}
